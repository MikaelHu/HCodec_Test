...
static int encode(AVCodecContext *ctx, AVFrame *frame, AVPacket *pkt, FILE *output) {
	...
	if (ret == AVERROR(EAGAIN)) {
		...
		fprintf(stderr, "Error failed to add packet to internal queue, or similar other errors : legitimate decoding errors\n");
		return ret;
	}
	else if (ret == AVERROR(EINVAL)) {
		fprintf(stderr, "Error codec not opened, refcounted_frames not set, it is a decoder, or requires flush\n");
		return ret;
	}
	else if (ret < 0) {
	...
	while (ret >= 0) {
		...
		if (ret == AVERROR(EAGAIN)) {
			fprintf(stderr, "Output is not available in the current state - user must try to send input\n");
			return 0;
		}
		else if (ret == AVERROR_EOF) {
			fprintf(stderr, "The encoder has been fully flushed, and there will be no more output packets\n");
			return 0;
		}
		else if (ret == AVERROR(EINVAL)) {
			fprintf(stderr, "Error codec not opened, refcounted_frames not set, it is a decoder\n");
			return ret;
		}
		else if (ret < 0) {
...
* 这里只支持2通道的转换
*/
void f32le_convert_to_fltp(uint32_t *f32le, uint32_t *fltp, int nb_samples) {
	uint32_t *fltp_l = fltp;   // 左通道
	uint32_t *fltp_r = fltp + nb_samples;   // 右通道
...
//
//
//	char* str_pcm = "F:/Projs/WorkDir/Projects/Nico_Face_Reco/audio_data/sample-15s_nc_2_44100.pcm";
//	char* str_aac = "F:/Projs/WorkDir/Projects/Nico_Face_Reco/audio_data/sample-15s_nc_2_44100.aac";
//
//	in_pcm_file = str_pcm;      // 输入PCM文件
//	out_aac_file = str_aac;     // 输出的AAC文件
//
...
//			f32le_convert_to_fltp((uint32_t*)pcm_buf, (uint32_t*)pcm_temp_buf, frame->nb_samples);
//
//			ret = av_samples_fill_arrays(frame->data, frame->linesize,
